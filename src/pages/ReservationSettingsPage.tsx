...
export default function ReservationSettingsPage() {
  ...
  const saveSettings = async () => {
    ...
    try {
      const r = await getRestaurant()
      await updateRestaurantSettings({ ...r.settings, reservation_settings: {
    ...
    <div className="space-y-8 px-8 md:px-16 lg:px-24">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="relative">
          ...
          <div className="relative rounded-2xl border border-white/10 bg-[#0C1020]/80 p-6">
            <h3 className="text-lg font-semibold mb-4">General</h3>
            <div className="space-y-4">
              <label className="block text-sm">Minimum Lead Time (hours)</label>
              <input type="number" className="w-full rounded-md bg-black/20 border border-white/10 p-2" value={leadTimeHours} onChange={e => setLeadTimeHours(parseInt(e.target.value || '0'))} />
              <label className="block text-sm">Daily Cutoff Time</label>
              <input type="time" className="w-full rounded-md bg-black/20 border border-white/10 p-2" value={cutoffTime} onChange={e => setCutoffTime(e.target.value)} />
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {['Birthday','Anniversary','First Date','Candlelight Dinner','Business Meeting','Family Gathering','Proposal','Celebration'].map(o => (
                    <button key={o} onClick={() => setSpecialOccasions(prev => prev.includes(o) ? prev.filter(x => x !== o) : [...prev, o])} className={`px-2 py-1 rounded-md border ${specialOccasions.includes(o) ? 'bg-primary/20 border-primary/30' : 'bg-black/20 border-white/10'}`}>{o}</button>
                  ))}
                </div>
              </div>
            </div>
        ...
        <div className="relative">
          ...
          <div className="relative rounded-2xl border border-white/10 bg-[#0C1020]/80 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <select value={day} onChange={e => setDay(parseInt(e.target.value))} className="rounded-md bg-black/20 border border-white/10 p-2">
                {DAYS.map((d, i) => <option key={d} value={i}>{d}</option>)}
              </select>
            </div>
            <div className="space-y-2">
              {slots.map(s => (
                <div key={s.id} className="flex items-center justify-between rounded-md border border-white/10 p-3">
                  <div className="text-sm">{s.start_time} - {s.end_time} (max {s.max_reservations})</div>
                  <div className="flex gap-2">
                    <button onClick={() => setEditing(s)} className="px-2 py-1 rounded-md border border-white/10 bg-black/20">Edit</button>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0C1020]/90 p-6">
            <h4 className="text-lg font-semibold mb-4">{(editing as any).id ? 'Edit Slot' : 'Add Slot'}</h4>
            <div className="space-y-3">
              <label className="block text-sm">Start Time</label>
              <input type="time" value={editing.start_time || ''} onChange={e => setEditing({ ...editing, start_time: e.target.value })} className="w-full rounded-md bg-black/20 border border-white/10 p-2" />
              <label className="block text-sm">End Time</label>
              <input type="time" value={editing.end_time || ''} onChange={e => setEditing({ ...editing, end_time: e.target.value })} className="w-full rounded-md bg-black/20 border border-white/10 p-2" />
              <label className="block text-sm">Max Reservations</label>
              <input type="number" value={(editing.max_reservations as any) || 10} onChange={e => setEditing({ ...editing, max_reservations: parseInt(e.target.value || '0') } as any)} className="w-full rounded-md bg-black/20 border border-white/10 p-2" />
            </div>
            <div className="mt-6 flex justify-end gap-2">
              <button onClick={() => setEditing(null)} className="px-3 py-2 rounded-md border border-white/10 bg-black/20">Cancel</button>
